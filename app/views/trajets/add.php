<div class="user-form-container">
    ...
    <form id="addTrajetForm" class="elegant-form" method="POST" action="index.php?action=storeTrajet">
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <input type="number" id="tarif" name="tarif" placeholder="Ex : 2000" required>
                <div class="error-message">Veuillez indiquer un tarif</div>
            </div>

            <div class="form-group">
                <label for="nbre_tickets_dispo">Nombre de tickets disponibles</label>
                <input type="number" id="nbre_tickets_dispo" name="nbre_tickets_dispo" required>
            ...
            <div class="form-group">
                ...
                <select id="bus_id" name="bus_id" required>
                    <option value="" disabled selected hidden>Sélectionner un bus...</option>
                    <?php foreach ($buses as $bus): ?>
                        <option value="<?= $bus['id'] ?>"><?= htmlspecialchars($bus['immatriculation']) ?></option>
                    <?php endforeach; ?>
                </select>
                <div class="error-message">Veuillez sélectionner un bus</div>
            </div>

            <div class="form-group">
                <label for="conducteur_id">Conducteur</label>
                <select id="conducteur_id" name="conducteur_id" required>
                    <option value="" disabled selected hidden>Sélectionner un conducteur...</option>
                ...
                <div class="error-message">Veuillez sélectionner un conducteur</div>
            </div>
        </div>
...
<script>
document.getElementById('addTrajetForm').addEventListener('submit', function(e) {
    let isValid = true;
